...
class Warehouse(gym.Env):
    ...
    def save_env_image(self, save_dir="results/env_images", filler=False):
        """Save the rendered environment image to disk.
        
        Args:
            save_dir: Directory where the image will be saved
            filler: Whether to skip drawing (use existing buffer)
        """
        if not self.renderer:
            from pymarlzooplus.envs.robotics_warehouse_v1.rware_v1.rendering import Viewer
            self.renderer = Viewer(self.grid_size)
        self.renderer.save_env_image(self, save_dir=save_dir, filler=filler)
