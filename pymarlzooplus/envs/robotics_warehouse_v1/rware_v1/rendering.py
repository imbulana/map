...
class Viewer(object):
    ...
    def save_env_image(self, env, save_dir="results/env_images", filler=False):
        """Save the rendered environment image to disk.
        
        Args:
            env: The environment to render and save
            save_dir: Directory where the image will be saved
        """
        os.makedirs(save_dir, exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')
        filename = f"{save_dir}/rware_render_{timestamp}.png"

        if not filler:
            # Draw the environment to the buffer
            ...
            # Construct RGB array from the buffer
            ...
            # save filler image
            filler_image = np.zeros((self.height, self.width, 3), dtype=np.uint8)
            cv2.imwrite(filename[:-3]+'_filler.png', filler_image)
