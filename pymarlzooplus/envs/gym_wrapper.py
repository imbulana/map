...
class _GymmaWrapper(MultiAgentEnv):
    ...
    def save_env_image(self, save_dir="results/env_images/10.025", filler=False):
        """Save the rendered environment image to disk.
        
        Args:
            save_dir: Directory where the image will be saved
            filler: Unused parameter for compatibility
        """
        # Try to call save_env_image on the underlying environment if it exists
        if hasattr(self._env.unwrapped, 'save_env_image'):
            self._env.unwrapped.save_env_image(save_dir=save_dir, filler=filler)
        elif hasattr(self._env.unwrapped, 'renderer') and hasattr(self._env.unwrapped.renderer, 'save_env_image'):
            self._env.unwrapped.renderer.save_env_image(self._env.unwrapped, save_dir=save_dir, filler=filler)
